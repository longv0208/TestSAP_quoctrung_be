...
| PUT | `/api/contracts/{id}` | Admin, Staff | **Sửa hợp đồng (mới 05/08)** — `UpdateContractRequest` { contractNumber, scopeTitle?, startDate, endDate, maxExtensionMonths, sideARepresentative?, econtractUrl? }. KHÔNG đổi được `proposalId` và `totalAmount`. Trùng số HĐ → 409; `endDate <= startDate` hoặc `maxExtensionMonths < 0` → 400. Hợp đồng **chưa từng gia hạn** thì sửa `endDate` dời luôn `originalEndDate` (là sửa cho đúng, không phải gia hạn); đã gia hạn rồi thì giữ nguyên hạn gốc. |
