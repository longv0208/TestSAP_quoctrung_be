...
| A3 | **Thư ký cũng được chấm điểm** | [B1] | ✅ **XONG 09/08 — văn bản đã trả lời, không cần hỏi thầy** | Tra nguyên văn QĐ543: **Điều 8.3.b** *"các thành viên tham dự họp **cần đánh giá thẩm định** đề cương (theo Biểu mẫu 03)"* — không loại trừ ai; **Điều 8.3.c** gọi Thư ký là người *"ghi biên bản… và **các thành viên của Hội đồng** thông qua"* ⇒ Thư ký nằm trong "các thành viên". Rà code: `IsSecretary` chỉ xuất hiện **một chỗ** — trong `SaveMinutesAsync` (chỉ Thư ký soạn biên bản); **không có nhánh nào chặn Thư ký chấm**. Thêm test `SubmitScore_BySecretary_Succeeds` để **khoá hành vi**: vai Thư ký đã bị chặn ở chỗ khác nên rất dễ có người thấy `IsSecretary` rồi tiện tay chặn nốt — chặn nhầm là **thiếu một phiếu**, kéo theo hụt quorum 2/3 và cả hội đồng không chốt được biên bản. Kiểm trên trình duyệt: bảng BM12 hiện phiếu của Thư ký (TS. Phạm Thu Hương 84/100). |
...
| A12 | **Hội đồng NGHIỆM THU không chốt được biên bản dù cả hội đồng đã bỏ phiếu** | *(tự phát hiện khi dựng data E7)* | ✅ **XONG 08/08** | Vòng xét duyệt nộp phiếu điểm vào `review_scores`, vòng nghiệm thu nộp phiếu Đạt/Không đạt vào `acceptance_evaluations` (BM11 không có thang điểm) — nhưng `AssertQuorumAsync` **chỉ đếm bảng đầu**. Hậu quả: hội đồng nghiệm thu đủ 5/5 phiếu vẫn báo *"mới có 0/5 phiếu"*, **không ai chốt được biên bản nghiệm thu**. Lỗi này chỉ lộ ra khi có dữ liệu đứng đúng ở bước nghiệm thu — nếu không làm E7 thì nó nổ trên sân khấu. Nay quorum gộp **cả hai bảng** (distinct theo thành viên), rule *"nghiệm thu phải có phản biện dự"* (Điều 12.3.b) cũng nhận phiếu BM11. Kiểm bằng API: 3/5 → chặn đúng câu QĐ543 · bỏ phiếu thứ 4 → lưu được. |
| A13 | **Biên bản nghiệm thu hiện "0 phiếu hợp lệ"** | *(cùng gốc A12)* | ✅ **XONG 08/08** | `ComputeTallyAsync` cũng chỉ đếm `review_scores` ⇒ số liệu BM12 mục 10.1 của **mọi** biên bản nghiệm thu đều bằng 0, Thư ký không có số để điền. Nay gộp phiếu Đạt/Không đạt vào ô *thu về / hợp lệ / không hợp lệ*, còn *điểm trung bình* để **null** vì nghiệm thu không chấm điểm. Kiểm bằng API: phát ra 5 · thu về 4 · hợp lệ 4 · TB null. |
| A14 | **Seeder hỏng là sập cả API** — và bộ tiêu chí 125 điểm trên deploy làm gãy màn chấm | *(tự phát hiện khi giả lập deploy)* | ✅ **XONG 08/08** | Giả lập đúng tình huống bạn trong nhóm deploy code lên **DB đang chạy, không xoá**: (1) `DemoScenarioSeeder` gặp trùng số hợp đồng → ném lỗi ra `Program.cs` → **ứng dụng không khởi động**, FE mất luôn backend; (2) seeder lấy bộ REVIEW active đầu tiên, mà deploy đang có bộ **125 điểm** ⇒ chốt chặn A11 chặn nộp phiếu ⇒ **màn chấm điểm gãy trên deploy dù local chạy ngon**; (3) lỗi giữa chừng để lại **nửa đề tài đã ghi**, lần khởi động sau thấy mã đã tồn tại nên bỏ qua luôn, rác nằm lại vĩnh viễn. Nay: mỗi kịch bản chạy trong **một giao dịch riêng** (lỗi → rollback sạch → cảnh báo → kịch bản khác vẫn dựng) · `Program.cs` bọc try/catch · seeder **tự tạo bộ BM03 hợp lệ** thay vì tin bộ có sẵn (bộ cũ giữ nguyên, không sửa của người dùng) · số hợp đồng trùng thì né hậu tố. Kiểm: dựng DB giả lập có bộ 125 điểm + trùng số hợp đồng → app **vẫn khởi động**, log đúng *"Bỏ qua phần dữ liệu demo…"*, số project **không đổi** (rollback sạch), sinh thêm bộ BM03 100/100. |
| A15 | **Lỗi nhắc hạn giết cả tiến trình** | *(gặp thật trên máy user)* | ✅ **XONG 08/08** | Khi cổng 5068 bị chiếm, host tắt giữa chừng → lần quét nhắc hạn mất kết nối DB → EF ghi log lỗi → provider **EventLog đã dispose** → `ObjectDisposedException` → khối `catch` của `DeadlineReminderService` gọi `LogError` **lại ném tiếp** → thoát khỏi `ExecuteAsync` → .NET 8 mặc định `StopHost` ⇒ **sập cả tiến trình** (exit `0xe0434352`). Nay bọc mọi lời gọi log bằng `SafeLog` + chặn exception ngay trong `ExecuteAsync`. Việc nhắc hạn hỏng thì thôi, không được đánh sập API. |
...
| C3 | **Trường của BÊN B**: tài khoản ngân hàng, CCCD | [B2-9] | ✅ **XONG 08/08** | Migration `PhaseN_PiContractIdentity` thêm 5 cột vào `academic_profiles`. Endpoint chỉ có đường **`/me`** ⇒ **chính chủ tự khai**, Staff không gõ hộ. Đọc ra **luôn che** `****1234`; số đầy đủ **chỉ** đổ vào file Word hợp đồng lúc xuất. Mỗi lần đổi ghi `audit_logs` với `old_values` **đã che** và `new_values` chỉ ghi tên trường — không nhân bản số thật sang bảng khác. **TUỲ CHỌN** đúng như đã chốt: `missingForContract[]` chỉ để nhắc, chưa khai thì hợp đồng vẫn lập được và bản Word chấm lửng như bản giấy. Phải sửa kèm: `GET/PUT /users/{id}/profile` trước đây trả **thẳng entity** ⇒ thêm cột là lộ nguyên số cho Admin/Staff; nay trả DTO (tên trường giữ y hệt, FE không phải sửa). FE: card "Thông tin để lập hợp đồng" ở trang hồ sơ cá nhân, ô nhập **không đổ sẵn số đã che** (sửa trên `****7890` là lưu nhầm dấu sao thành số thật). Kiểm luồng thật: PI khai → đọc lại bị che → Staff xem hồ sơ **không** thấy số → bản Word có số đầy đủ → nhật ký ghi đúng. |
| C4 | **Hồ sơ nghiệm thu phải có: link sản phẩm + TOÀN BỘ thông tin sản phẩm và đề tài + file Word/PDF** | [B1] | ✅ **XONG 08/08** | Dossier nay trả khối **`project`** đầy đủ (mã · tên VI/EN · PI + email · đơn vị chủ trì · loại · lĩnh vực · đợt · thời gian · tổng kinh phí · **mục tiêu/phương pháp/sản phẩm dự kiến** · danh sách thành viên · file đề cương gốc) + thông tin hợp đồng (ngày ký, thời hạn, giá trị, **file bản ký**). Sản phẩm thêm `fileUrl` (link PI dán) · `trialEvidenceUrl` (minh chứng thử nghiệm, Điều 13.1) · `scientificRequirements`. Báo cáo tổng kết thêm `reportFileUrl`/`summaryFileUrl`. Quan trọng nhất: **mọi mục đều kèm `files[]` có `downloadUrl` thật** — trước chỉ có cờ `hasFile`, người chấm biết là có file mà không mở được. Kiểm bằng API với reviewer thật trên `NCKH-2026-007`. |
| C5 | **Hồ sơ nghiệm thu phải chi tiết TỪNG LẦN báo cáo tiến độ**: ai chấm · role gì · bao nhiêu điểm · xem lại được **tất cả file của các lần trước** | [B2-14] | ✅ **XONG 08/08** (trừ "điểm" — không tồn tại) | Mỗi kỳ nay trả `evaluatedByName` · `evaluatedByRole` · `evaluatedAt` · nội dung **đã làm / còn tồn / kế hoạch kỳ sau / kiến nghị của PI** · `reportFileUrl` · **`files[]` của CHÍNH kỳ đó** (lấy một lượt, không N+1) ⇒ xem lại được tất cả các kỳ trước chứ không chỉ kỳ cuối. ⚠️ **"Bao nhiêu điểm" thì không có và không nên có**: rule #16 (chốt tuần 10) — báo cáo tiến độ giữa kỳ **Staff duyệt trực tiếp, KHÔNG lập hội đồng** nên không có thang điểm. DTO trả kèm `progressReportNote` nói rõ để người chấm không đi tìm cột điểm không tồn tại. Nếu thầy muốn có điểm thật thì phải bàn lại rule #16 trước, không tự thêm. |
| C6 | **Chưa có giải ngân đợt CUỐI trước khi chốt nghiệm thu (kết thúc hợp đồng)** | [B2-16] | ✅ **XONG 08/08** | Nối 2 khoá: (1) **đợt CUỐI chỉ chi được sau khi đề tài được công nhận Đạt** — đọc qua `Project.Status == COMPLETED`, trạng thái này chỉ có MỘT đường đặt là Chủ tịch chốt biên bản nghiệm thu Đạt; (2) **còn đợt nào chưa chi thì không lập được quyết toán** (quyết toán = bước đóng hợp đồng; không chặn thì hợp đồng đóng xong vẫn còn đợt treo không còn đường chi). ⚠️ Bản đầu viết luật quá rộng, **test đỏ 2 ca** lôi ra: hợp đồng chỉ có **1 đợt** thì đợt đó vừa đầu vừa cuối, chặn nó là cấm luôn tiền tạm ứng sau khi ký ⇒ luật chỉ áp khi hợp đồng có **≥2 đợt**. Sửa luật chứ không sửa test. Thêm 4 test: chặn đúng đợt cuối khi đề tài IN_PROGRESS/ACCEPTANCE · các đợt trước vẫn chi bình thường · sau khi COMPLETED thì chi được. |
...
| D3 | **Báo cáo tổng kết mới chỉ có link sản phẩm — phải thêm file Word/PDF** | [B1] | ✅ **XÁC NHẬN XONG 09/08** | Rà lại `FinalReportPanel`: cả bản chính lẫn bản tóm tắt đều là **ô upload file thật** (.pdf/.doc/.docx), upload xong lấy `downloadUrl` của hệ thống làm `reportFileUrl` — **không còn ô dán link nào**. Việc này thực ra đã làm 31/07, chỉ thiếu bước xác nhận nên treo ở 🔶. |
...
| D6 | **Ô nhập thông tin proposal phải tải lên được từ file Word** | [B2-19] | ✅ **hoàn thiện 18/08** | Đường B dùng `POST /api/proposals/extract`: PDF/DOCX → AI trích xuất → chỉ prefill ô trống; nút và tiến trình hiện rõ ở bước Nội dung nghiên cứu, file gốc được đính kèm khi lưu nháp. |
...
| # | Việc | Nguồn | Trạng thái | Ghi chú thực thi |
|---|---|---|---|---|
| E1 | **Lĩnh vực: cho chọn NHIỀU lĩnh vực cùng lúc**, thay vì chọn 1 → OK → chọn tiếp | [B1] | ✅ **XONG 09/08** | `ManageCycleFieldsDialog` đổi từ dropdown-chọn-một sang **danh sách tick nhiều**, nút hiện *"Gắn N lĩnh vực"*, gắn **tuần tự** (mỗi lĩnh vực một yêu cầu — bắn song song thì lỗi ở giữa khó truy). Đợt mở 4 lĩnh vực trước đây phải lặp 4 vòng. |
| E2 | **Sửa lại cách hiển thị năm học** — `2025-2026`, hoặc chỉ 1 năm dương `2026` | [B1] | ✅ **XONG 08/08** | Hoá ra BE **vốn đã đúng**: `CycleYear` là `int`, `CreateCycleAsync` chỉ nhận số nguyên. Sai nằm ở FE — nhãn ghi **"Năm học"** và placeholder gợi ý **`2025-2026`**, tức là *gõ đúng như hệ thống gợi ý thì API trả 400*. Nay: nhãn → **"Năm"** / "Year", placeholder → **`2026`** (kèm `inputMode=numeric`, `maxLength=4`), zod chặn tại chỗ bằng regex 4 chữ số kèm câu chỉ đường. Căn cứ: QĐ543 dùng **năm dương lịch** (*"Quý I hằng năm…"*, biểu mẫu ghi *"NĂM 20…"*), không có khái niệm năm học. Giữ nguyên tên trường `academicYear` trên đường truyền — đổi tên là breaking change cho FE↔BE, không đáng làm sát ngày bảo vệ. |
...
| E7 | **Chuẩn bị DATA DEMO chuẩn thật**: sẵn 1 đợt có **nhiều đề tài**, **nhiều proposal đã nộp** | [B2-3] | ✅ **XONG 08/08** | `DemoScenarioSeeder` chạy sau seeder gốc, idempotent theo `ProjectCode`, tắt bằng setting `DEMO_DATA_ENABLED`. Dựng **2 đợt** (UD26 mở / CB26 đóng) + **8 đề tài `NCKH-2026-001..008`** đứng ở 8 bước khác nhau, **3 hội đồng × 5 người** (số lẻ, đủ Chủ tịch/Thư ký/Phản biện, đã xác nhận, đã có lịch họp), **#3+#4+#5 chung 1 hội đồng — 3 slot con 45′** để lịch chấm có cái mà xem, 14 phiếu chấm + 8 phiếu nghiệm thu + 3 biên bản đã chốt, 3 hợp đồng + 4 báo cáo tiến độ + quyết toán, và **6 file Word thuyết minh sinh sẵn** (`ExportScientificDocAsync` → `IFileStorage` → `Document`) để demo AI đọc file chạy thật. Đổi luôn tên tài khoản placeholder ("PGS.TS Lê Phản Biện" → tên người thật-nghĩa) và thêm reviewer4/5 vì hội đồng cần 5 người. **Kiểm bằng DB thật đã xoá sạch**: 10 project · rubric 100/100 · 5 slot · 6 doc; và bằng API: Thư ký lưu được biên bản #3 (4/5 phiếu, TB 87.25), bị chặn ở #7 (3/5) rồi qua sau phiếu thứ 4. |
| E8 | **Sửa mật khẩu tất cả tài khoản thành `password`** | [B2-13] | ✅ **XONG 09/08** | Hằng `DatabaseSeeder.DemoPassword`, áp cho cả 9 tài khoản demo. Phải thêm bước **`ResetDemoPasswordsAsync`** vì seeder chỉ đặt mật khẩu lúc TẠO — cơ sở dữ liệu đã chạy từ trước thì đổi hằng số cũng vô tác dụng, đứng trước hội đồng mới phát hiện vẫn là chuỗi cũ. Chỉ chạm đúng 9 email demo, **không đụng** tài khoản người dùng thật, và chỉ chạy khi `DEMO_DATA_ENABLED` bật. Sửa mật khẩu trong **7 file doc**. ⚠️ `password` cố tình dễ — bản deploy công khai phải đổi mật khẩu admin hoặc tắt setting. Kiểm bằng API trên DB **đang có**: 5 tài khoản đăng nhập được bằng `password`, mật khẩu cũ bị từ chối. |
...
| # | Việc | Nguồn | Trạng thái | Ghi chú thực thi |
|---|---|---|---|---|
| F1 | 🔴 **VALIDATE LẠI TOÀN BỘ** — rà mọi ràng buộc | *(cả 2 bản)* [B1] · [B2] | ✅ **XONG 09/08** | Rà **277 chốt chặn** trong service/controller, đối chiếu với rule #1–#23. Ra **5 lỗi thật**: ① `Dimension` vẫn nhận `FINANCE` dù rule #16 đã bỏ; ② chốt *"phải ĐẠT vòng tiên quyết"* gắn với `Dimension == FINANCE` ⇒ thành **code chết**, đề tài trượt vòng xét duyệt vẫn gán được vào vòng nghiệm thu; ③ hai đường cùng làm việc đó xử lý *"chưa từng tham gia vòng trước"* **khác nhau** (một chặn, một cho qua); ④ luật *"Staff phải xem được bản báo cáo mới đánh giá"* **chỉ khoá ở giao diện** — gọi thẳng API là đánh giá được báo cáo trắng trơn; ⑤ seeder demo ghi `EvaluationResult = "ACHIEVED"` trong khi BE chỉ nhận PASS/CONDITIONAL/FAIL (**lỗi của chính mình**, DB đã reseed). Kèm theo: dịch **86 + 2 câu lỗi nghiệp vụ còn tiếng Anh** ⇒ nay **0 câu tiếng Anh** trong toàn bộ service (bổ sung cho E3). Thêm 4 test. |
| F2 | **Thiết kế lại chi tiết business rule** (5 điểm bản 2 nêu) | [B2-BR] | ✅ **XONG 09/08** | Viết **`docs/BUSINESS_RULES.md`** — bảng tra **luật → căn cứ (điều/biểu mẫu QĐ543 hoặc buổi chốt) → nơi thực thi trong code → mã lỗi**, chia 6 nhóm A–F, **62 luật**. Cả 5 điểm bản 2 nêu đều có số hiệu: ① C3 ② C8 ③ C5 ④ D2 ⑤ C1. Thêm hai mục hay bị hỏi đi hỏi lại: **"chỗ đã rà và KHÔNG có luật"** (để khỏi rà lại) và **độ phủ kiểm thử theo nhóm** kèm danh sách chỗ còn mỏng. Không nhét vào `CLAUDE.md` vì file đó là nơi ghi **quyết định**; `BUSINESS_RULES.md` là bản **soi chiếu xuống code** — lệch nhau thì `CLAUDE.md` đúng. |
...
| F4 | **Phụ lục** | [B1] | ✅ **XONG 08/08** — hiểu là **phụ lục HỢP ĐỒNG** | `GET /api/amendments/{id}/export-word` sinh .docx phụ lục cho đề nghị điều chỉnh **đã duyệt**, đem ký ngoài rồi upload bản ký làm minh chứng (giống luồng hợp đồng gốc, rule #21). Lý do cần: hợp đồng đã ký thì **không sửa đè lên bản gốc** — trước đây duyệt điều chỉnh xong chỉ đổi vài dòng trong DB, không có giấy tờ nào đem ký, nên hồ sơ quyết toán không giải thích được vì sao thời gian/nội dung khác hợp đồng gốc. Nội dung: căn cứ QĐ543 + hợp đồng gốc + **Điều 6.1** (báo trước 15 ngày) · Bên A/Bên B · Điều 1–4 · **bảng trước → sau** · ô ký. Với gia hạn, DB lưu `NewValue` là SỐ THÁNG — in trần *"0 → 3"* vào văn bản đem ký thì vô nghĩa, nên quy ra **mốc thời gian thật** (`01/03/2026 – 01/03/2027` → `01/03/2026 – 01/06/2027`) kèm dòng *"Số tháng gia hạn: 3 tháng"*. Chặn **409** nếu đơn chưa duyệt (in bản chờ duyệt = giấy tờ khống). Thêm 3 test; kiểm bằng file .docx xuất thật. |
| F5 | **Tạo rõ KỊCH BẢN để demo sản phẩm** | [B1] | ✅ **XONG 08/08** | Viết vào **`docs/DEMO_GUIDE.md`** (§2 tài khoản · §3 bảng 8 đề tài · §4 kịch bản ①→⑧) thay vì đẻ thêm `DEMO_SCRIPT.md` — cẩm nang capstone cảnh báo đúng chuyện doc chồng doc. Mỗi mục ①→⑧ **độc lập**, mở thẳng vào là diễn được. |
...
- Mật khẩu tất cả tài khoản = `password` (E8). ⚠️ Hiện **không đồng nhất**: `reviewer3.demo` dùng
  `password` còn `pi.demo` dùng khác — sửa thì phải sửa cả doc đang chép mật khẩu cũ.
