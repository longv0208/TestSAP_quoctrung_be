...
- ✅ **04/08 — thông báo qua EMAIL (`INotifier`):** gom "tạo chuông + gửi mail" về **một chỗ dùng chung**, thay vì mỗi service tự nhớ.
  ...
  - **Test được luồng mail:** thêm `EmailSettings:RedirectAllTo` — dev thì hứng mail của các miền giả về 1 hộp thư thật (tài khoản seed dùng email không tồn tại; đổi email seed sẽ hỏng seeder vì nó dùng email làm **khóa định danh**). Tiêu đề ghi `[→ người-nhận-thật]`; `email_log` vẫn lưu người nhận thật. **PROD để trống.**
  - **Sửa 12/08 — mail thật từng bị nuốt:** ban đầu `RedirectAllTo` chuyển hướng **TẤT CẢ**, nên tạo tài khoản bằng địa chỉ thật thì người ta không hề nhận được thư, mà `email_log` vẫn ghi `SENT` ⇒ không có dấu hiệu nào để lần ra. Nay thêm `EmailSettings:RedirectDomains` (mặc định `["furpms.edu.vn"]`): **chỉ miền giả mới bị hứng**, địa chỉ thật đi thẳng. Đo thật: thư `PASSWORD_RESET` tới `@fpt.edu.vn` không còn tiền tố `[→ …]` trong `email_log`.
