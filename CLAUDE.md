...
15. **Hệ thống quản HỒ SƠ KINH PHÍ, không quản DÒNG TIỀN.** *(sửa lại 25/08 sau biên bản bảo vệ lần 2 — xem `docs/KE_HOACH_BAO_VE_LAN2.md` §3-T1)*
    - **CÓ hiển thị đầy đủ:** dự toán được duyệt (Điều 15) · trần theo loại đề tài · **giá trị hợp đồng** · lịch giải ngân theo %/mốc · trạng thái từng đợt · quyết toán. Đây là **hồ sơ hành chính của đề tài**, hội đồng đòi phải thấy rõ.
    - **KHÔNG làm:** không thực hiện thanh toán, không nối ngân hàng, không thay sổ kế toán. Kế toán chi tiền ngoài hệ thống; `ActualAmount` là **ghi nhận lại** con số Phòng Tài chính báo, kèm minh chứng — hệ thống không tự tính, không tự chi.
    - ⚠️ **Câu cũ ("không quản tiền", scope = *strip+ẩn*, "ẩn nav/UI tiền") đã bị hiểu thành "không được HIỆN tiền"** nên `Contract.TotalAmount` bị giấu khỏi mọi màn dù BE vẫn trả về — **hội đồng bảo vệ lần 2 bắt đúng chỗ này**. Ẩn *cấu hình* tài chính thì đúng; ẩn *số tiền của đề tài* thì sai.
    - **→ Rule #2, #3, #6 (giải ngân/FINANCE cũ) vẫn SUPERSEDED.** Giữ bảng, không migration bỏ entity. (Đã làm: `disbursement/{id}/evidence`, confirm→optional amount.)
